...
device = torch.device("cuda" if cuda_is_present else "cpu")
...
for epoch in range(1, args.epochs+1):
	for i, (images, _) in enumerate(data_loader):
		...
		real_output = torch.full((images.size(0), 1), 1.0, device=device, requires_grad=False)
		fake_output = torch.full((images.size(0), 1), 0.0, device=device, requires_grad=False)
