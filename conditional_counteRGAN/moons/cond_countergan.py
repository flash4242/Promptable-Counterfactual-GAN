...
parser.add_argument('--target', type=int, default=1, help='Cél label az ellenpéldához (0 vagy 1). Default: 1.')
...
class Generator(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=32, cond_dim=1):
        super().__init__()
...
for epoch in range(params['epochs']):
    ...
    # === Discriminator ===
    x_cf = G(x, target_label)
    d_real = D(x, y_true)
    d_fake = D(x_cf.detach(), target_label)
...
G.eval()
with torch.no_grad():
    x_cf = G(x_orig, target_cf_label).cpu()
...
plt.grid(True)
plt.savefig("cond_countergan_moons.png")