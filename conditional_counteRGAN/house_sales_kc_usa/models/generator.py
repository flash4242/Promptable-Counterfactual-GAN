...
class ResidualGenerator(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes):
        super().__init__()
        self.fc_in = nn.Linear(input_dim + num_classes + input_dim, hidden_dim)
        ...
        self.res5 = ResidualBlock(hidden_dim)
        self.fc_out = nn.Linear(hidden_dim, input_dim)

    def forward(self, x, target, mask=None):
        x_input = torch.cat([x, target, mask], dim=1) if mask is not None else torch.cat([x, target, torch.ones_like(x)], dim=1)
